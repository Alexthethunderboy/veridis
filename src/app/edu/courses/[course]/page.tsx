...
import { estimateLessonMinutes } from '@/lib/data/lessonMetrics';
import LearningProgress from '@/components/learning/LearningProgress';
import CourseCompletionSummary from '@/components/learning/CourseCompletionSummary';
    ...
    <div className="mt-14 grid gap-12 lg:grid-cols-[minmax(0,1fr)_280px]">
      <div className="space-y-14">{course.modules.map((module, moduleIndex) => <section key={module.title}>
        ...
        <div className="overflow-hidden rounded-2xl border border-brand-primary/10">{module.lessons.map((lesson, lessonIndex) => <Link key={lesson.slug} href={`/edu/courses/${course.slug}/${lesson.slug}`} className="group flex gap-4 border-b border-brand-primary/10 bg-brand-stone-100 p-5 last:border-0 hover:bg-brand-stone-200 sm:items-center sm:p-6">
          <span className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-brand-primary/15 text-xs font-bold text-brand-primary/40 sm:mt-0">{lessonIndex+1}</span><div className="min-w-0 flex-1"><h3 className="font-bold group-hover:text-brand-emerald-900">{lesson.title}</h3><p className="mt-1 line-clamp-2 text-sm leading-6 text-brand-primary/45">{lesson.summary}</p><p className="mt-2 text-[10px] font-bold uppercase tracking-wider text-sky-300">{lesson.sections.length+(CURRICULUM_ENRICHMENTS[lesson.slug]?.length??0)} teaching units · {CURRICULUM_ENRICHMENTS[lesson.slug]?.length??0} source-derived</p></div><span className="hidden items-center gap-1 text-xs text-brand-primary/35 sm:flex"><Clock3 size={13}/>{estimateLessonMinutes(lesson)} min</span><ArrowRight className="shrink-0 text-brand-primary/25 group-hover:text-brand-emerald-900" size={18}/>
        </Link>)}</div>
      </section>)}</div>
      <aside><div className="sticky top-8 space-y-4"><LearningProgress lessons={lessons.map((lesson)=>({slug:lesson.slug,title:lesson.title,href:`/edu/courses/${course.slug}/${lesson.slug}`}))}/><CourseCompletionSummary courseTitle={course.title} lessonSlugs={lessons.map((lesson)=>lesson.slug)}/><div className="rounded-2xl border border-brand-primary/10 bg-brand-stone-100 p-6"><p className="clinical-label text-brand-secondary">How to use this course</p><ol className="mt-5 space-y-4 text-sm leading-6 text-brand-primary/55"><li>1. Follow lessons in order if you are new.</li><li>2. Complete the mastery check in each lesson.</li><li>3. Save notes and inspect original sources.</li></ol></div></div></aside>
    </div>
  </main>;
}
